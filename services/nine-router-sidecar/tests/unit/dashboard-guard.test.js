...
describe("dashboard guard public LLM API access", () => {
  ...
  it.each(["/api/providers/catalog", "/api/providers"])(
    "allows custom-server authenticated tenant management route %s",
    async (pathname) => {
      const response = await proxy(request(pathname, {
        host: "localhost:20128",
        "x-9r-tenant-authenticated": "1",
      }));
  ...
  it("rejects an unsigned tenant management route", async () => {
    const response = await proxy(request("/api/providers/catalog", { host: "localhost:20128" }));

    expect(response.status).toBe(401);
    expect(response.body.error).toBe("Unauthorized");
  });

  it("does not let a tenant assertion bypass non-tenant host APIs", async () => {
