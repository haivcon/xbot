...
const RISK_LEVELS = [
    { id: 'conservative', icon: '🛡️', label: 'Conservative', desc: 'Low risk, smaller trades', color: 'emerald' },
    { id: 'moderate', icon: '⚖️', label: 'Moderate', desc: 'Balanced risk & reward', color: 'amber' },
    { id: 'aggressive', icon: '🔥', label: 'Aggressive', desc: 'High risk, bigger trades', color: 'red' },
];
                ...
                <div className="sticky top-0 z-10 bg-surface-900/95 backdrop-blur-sm border-b border-surface-700/30 p-4">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                            ...
                            <h2 className="text-base font-bold text-surface-100">AI Trader</h2>
                            <span className="px-1.5 py-0.5 text-[9px] font-bold bg-amber-500/20 text-amber-400 rounded border border-amber-500/30 uppercase tracking-wider">Beta</span>
                        </div>
                        <div className="flex items-center gap-2">
                ...
                <div className="p-4 space-y-4">
                    {loading ? (
                        ...
                        <div className="text-center space-y-6 py-8">
                            ...
                            <div>
                                <h3 className="text-lg font-bold text-surface-100 mb-2">AI Trading Agent</h3>
                                <p className="text-sm text-surface-200/60 leading-relaxed max-w-xs mx-auto">
                                    Let AI analyze market signals, create trade plans, and execute with your approval
                                </p>
                            </div>
                            <div className="grid grid-cols-3 gap-3 text-center">
                                <div className="p-3 rounded-xl bg-surface-800/40 border border-surface-700/30">
                                    ...
                                    <p className="text-[10px] text-surface-200/60">Signal Analysis</p>
                                </div>
                                <div className="p-3 rounded-xl bg-surface-800/40 border border-surface-700/30">
                                    <Shield size={18} className="mx-auto text-blue-400 mb-1.5" />
                                    <p className="text-[10px] text-surface-200/60">Risk Control</p>
                                </div>
                                <div className="p-3 rounded-xl bg-surface-800/40 border border-surface-700/30">
                                    <Target size={18} className="mx-auto text-amber-400 mb-1.5" />
                                    <p className="text-[10px] text-surface-200/60">Profit Target</p>
                                </div>
                            </div>
                        ...
                        <div className="space-y-5">
                            <div className="flex items-center gap-2 mb-1">
                                ...
                                <h3 className="text-sm font-bold text-surface-100">Configure Agent</h3>
                            </div>

                            {/* Risk Level */}
                            <div>
                                <label className="text-xs text-surface-200/60 mb-2 block">Risk Level</label>
                                <div className="grid grid-cols-3 gap-2">
                                    {RISK_LEVELS.map(r => (
                                        <button key={r.id} onClick={() => setRiskLevel(r.id)}
                                            ...
                                            <div className="text-[10px] font-semibold text-surface-100">{r.label}</div>
                                        </button>
                                    ))}
                                </div>
                            </div>
                            ...
                            <div>
                                <label className="text-xs text-surface-200/60 mb-1.5 block flex items-center gap-1">
                                    <DollarSign size={12} /> Total Budget (USD)
                                </label>
                            ...
                            {/* Max per trade */}
                            <div>
                                <label className="text-xs text-surface-200/60 mb-1.5 block">Max per Trade (USD)</label>
                            ...
                            {/* Chains */}
                            <div>
                                <label className="text-xs text-surface-200/60 mb-1.5 block">Chains</label>
                            ...
                            <label className="flex items-center justify-between p-3 rounded-lg bg-surface-800/30 border border-surface-700/30 cursor-pointer">
                                <div>
                                    ...
                                    <div className="text-[10px] text-surface-200/40">Execute trades without manual approval</div>
                                </div>
                            ...
                            <div className="flex items-start gap-2 p-3 rounded-lg bg-amber-500/5 border border-amber-500/20">
                                ...
                                <p className="text-[10px] text-amber-400/80 leading-relaxed">
                                    BETA: Trades are <strong>simulated</strong> in this version. Real execution coming soon. AI analyzes whale & smart money signals to generate trade proposals.
                                </p>
                            </div>
                            ...
                            <div className="flex gap-2">
                                ...
                                <button onClick={handleEnable} disabled={actionLoading === 'enable' || selectedChains.length === 0}
                                    ...
                                    {actionLoading === 'enable' ? <div className="animate-spin h-4 w-4 border-2 border-white border-t-transparent rounded-full" /> : <><Zap size={14} /> Start Agent</>}
                                </button>
                            </div>
                        </div>
                    ) : (
                            ...
                            <div className="p-4 rounded-xl bg-gradient-to-br from-surface-800/60 to-surface-800/30 border border-surface-700/30">
                                    ...
                                    <div className="flex items-center gap-1.5">
                                        {agentStatus?.paused ? (
                                            <button onClick={() => handlePause(false)} disabled={actionLoading === 'pause'}
                                                className="p-1.5 rounded-lg bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500/20 transition-colors" title="Resume">
                                                <Play size={12} />
                                            </button>
                                        ) : (
                                            <button onClick={() => handlePause(true)} disabled={actionLoading === 'pause'}
                                                className="p-1.5 rounded-lg bg-amber-500/10 text-amber-400 hover:bg-amber-500/20 transition-colors" title="Pause">
                                                <Pause size={12} />
                                            </button>
                                        )}
                                        <button onClick={handleDisable} disabled={actionLoading === 'disable'}
                                            className="p-1.5 rounded-lg bg-red-500/10 text-red-400 hover:bg-red-500/20 transition-colors" title="Stop Agent">
                                            <Power size={12} />
                                        </button>
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-3 gap-3 mb-3">
                                    <div className="text-center">
                                        <div className="text-[10px] text-surface-200/40 mb-0.5">Total PnL</div>
                                    ...
                                    <div className="text-center">
                                        <div className="text-[10px] text-surface-200/40 mb-0.5">Trades</div>
                                        <div className="text-sm font-bold text-surface-100">{agentStatus?.totalTrades || 0}</div>
                                    </div>
                                    <div className="text-center">
                                        <div className="text-[10px] text-surface-200/40 mb-0.5">Pending</div>
                                ...
                                <div>
                                    <div className="flex items-center justify-between mb-1">
                                        <span className="text-[10px] text-surface-200/40">Profit Target</span>
                            ...
                            <div className="space-y-2">
                                {plans.length === 0 ? (
                                    <div className="text-center py-8">
                                        ...
                                        <p className="text-xs text-surface-200/40">No trade plans yet</p>
                                        <p className="text-[10px] text-surface-200/30 mt-1">AI is scanning market signals...</p>
                                    </div>
                                ) : plans.map(plan => (
                                        ...
                                        <button onClick={() => setExpandedPlan(expandedPlan === plan.id ? null : plan.id)}
                                            ...
                                            <div className="flex items-center gap-2.5 min-w-0">
                                                ...
                                                <div className="min-w-0">
                                                    ...
                                                    <div className="text-[10px] text-surface-200/40 truncate">{plan.chainLabel} • Score: {plan.aiScore}/100</div>
                                                </div>
                                            </div>
                                        ...
                                        {expandedPlan === plan.id && (
                                            <div className="px-3 pb-3 space-y-2 border-t border-surface-700/20 pt-2">
                                                ...
                                                <div className="p-2 rounded-lg bg-brand-500/5 border border-brand-500/10">
                                                    <div className="text-[10px] text-brand-400/60 mb-0.5">💡 AI Analysis</div>
                                                    <p className="text-[11px] text-surface-200/70 leading-relaxed">{plan.aiReason || 'Signal detected'}</p>
                                                </div>

                                                {/* Details Grid */}
                                                <div className="grid grid-cols-2 gap-2 text-[10px]">
                                                    <div className="flex justify-between">
                                                        <span className="text-surface-200/40">Price</span>
                                                        <span className="text-surface-100 font-mono">${Number(plan.tokenPrice || 0).toPrecision(4)}</span>
                                                    </div>
                                                    <div className="flex justify-between">
                                                        <span className="text-surface-200/40">Target</span>
                                                        <span className="text-emerald-400">+{plan.targetPct}%</span>
                                                    </div>
                                                    <div className="flex justify-between">
                                                        <span className="text-surface-200/40">Stop Loss</span>
                                                        <span className="text-red-400">-{plan.stopLossPct}%</span>
                                                    </div>
                                                    <div className="flex justify-between">
                                                        <span className="text-surface-200/40">Score</span>
                                                ...
                                                {plan.status === 'pending' && (
                                                    <div className="flex gap-2 pt-1">
                                                        ...
                                                        <button onClick={() => handleApprovePlan(plan.id)}
                                                            ...
                                                            {actionLoading === `approve-${plan.id}` ? <div className="animate-spin h-3 w-3 border-2 border-emerald-400 border-t-transparent rounded-full" /> : <><Check size={12} /> Approve</>}
                                                        </button>
                                                    </div>
                                                )}
